...
## เปิดจากเครื่องอื่น (iPad, มือถือ, โน้ตบุ๊ก)

**สองด่าน และล้มเหลวหน้าตาเหมือนกัน** — ต้องผ่านทั้งคู่

### ด่านที่ 1: ที่อยู่ที่เครื่องนั้นไปถึงได้

HUD รันอยู่ใน OrbStack VM ซึ่งมี IP ภายในของตัวเอง (`192.168.139.181`)
**เครื่องอื่นในบ้านไปถึงไม่ได้** — แต่ OrbStack forward พอร์ตออกมาที่ **IP ของ Mac**
ให้แล้ว จึงต้องใช้ที่อยู่ของ Mac:

```
http://192.168.1.40:8765/hud/       ← ใช้ได้ทันที
https://192.168.1.40:8766/hud/      ← ต้องใช้อันนี้ถ้าจะใช้กล้องหรือตำแหน่ง
```

ดู IP ของ Mac ด้วย `ipconfig getifaddr en0` · ผ่าน Tailscale ก็ใช้ IP ของ Mac
ในเครือข่ายนั้นได้เหมือนกัน

> พอร์ต 8766 เป็น TLS ที่ออกใบรับรองเอง เบราว์เซอร์จะเตือนครั้งแรก — กดยอมรับครั้งเดียว

### ด่านที่ 2: ชื่อนั้นต้องอยู่ใน allowlist

ต่อให้รหัสถูก **WebSocket จะถูกปฏิเสธ** ถ้า origin ไม่อยู่ในรายการ:

```yaml
# server/config/server.yaml
security:
  extra_origin_hosts: ["192.168.1.40", "100.78.9.105", "192.168.139.181"]
```

ใส่ทุกที่อยู่ที่จะใช้เปิดจริง แล้ว restart

### ใส่รหัสยังไงถ้าไม่ได้เปิดจากลิงก์

เดิมทางเดียวคือ URL ที่มี `?token=` ซึ่งใช้ได้ตอนแปะลิงก์ แต่**ไร้ประโยชน์ทันที**
ที่เปิดจากบุ๊กมาร์กของเครื่องนั้นเองหรือพิมพ์ที่อยู่บนแท็บเล็ต — หน้าเว็บขึ้นมา
ทุก API ตอบ 401 และ**ไม่มีที่ให้ใส่รหัส**

ตอนนี้ HUD **ขึ้นกล่องให้ใส่รหัสเอง** เมื่อเจอ 401 หรือ WebSocket ถูกปฏิเสธ
(code 4401) · กล่องนั้นบอก**ที่อยู่ที่กำลังเปิดอยู่**ด้วย เพราะรหัสผิดกับ
origin ไม่อนุญาต ให้อาการเหมือนกันเป๊ะ แยกไม่ออกถ้าไม่บอก
